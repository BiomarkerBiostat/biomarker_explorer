#-----------------------------------------------------------------------------
# Purpose:  Function to draw "Median + IQR" plot
# Author:   Feiyang Niu
# Date:     April 8, 2016
#-----------------------------------------------------------------------------
...
# Draw "Median + IQR" plot with points
median_iqr_plot <- function(data, bmk_col, visit_col, group_col = '',
                            group_levs = NULL,
                            to_log = FALSE, bmk_name = '', alpha = 0.3,
                            xlab = '', ylab = '', main = 'Median + IQR',
                            xlim = NULL, ylim = NULL, reference_line = NULL,
                            x_tick = visit_col, jitter_factor = 0.5,
                            fnote_size = 0.8, fnote_text = '',
                            add_points = FALSE, add_legend = FALSE,
    ...
    if(!is_blank(x_tick)) {
        ...
        dots <- setNames(expr, c('med', 'lower', 'upper', 'visit', 'N', 'x_tick'))
    } else dots <- setNames(expr, c('med', 'lower', 'upper', 'visit', 'N'))
    smmry <- data %>%
    ...
    yrange <- ternary(add_points, range_na(data[[bmk_col]]),
                         range_na(c(smmry$med, smmry$lower, smmry$upper)))
    ...
    for(idx in seq_along(groups)) {
        ...
        med <- sub_smmry$med
        lower <- sub_smmry$lower
        upper <- sub_smmry$upper
        N <- sub_smmry$N
